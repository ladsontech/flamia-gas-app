...
import { Loader2, Mail, Lock, User, ArrowLeft, Store } from 'lucide-react';
...
export default function StorefrontAuth() {
  ...
  const checkAuth = async () => {
    ...
    if (user) {
      // User is already logged in, redirect to storefront
  ...
  const handleGoogleSignIn = async () => {
    try {
      setLoading(true);
      const returnTo = searchParams.get('return_to') || window.location.origin;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-orange-50 flex flex-col">
      <Helmet>
        <title>Sign In - {shop?.shop_name || 'Store'} | Flamia</title>
      </Helmet>

      {/* Header */}
      <header className="bg-white border-b border-orange-100 py-4">
        ...
        <Card className="w-full max-w-md shadow-lg">
          <CardHeader className="space-y-1">
            ...
            <CardDescription className="text-center">
              Sign in to manage your store and access your dashboard
            </CardDescription>
          </CardHeader>
          <CardContent>
